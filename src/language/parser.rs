...
use super::{
    ...
    interpreter::Program, operator::Operator, token::Token,
};
...
pub struct Parser<'a> {
    path: PathBuf,
    lexer: itertools::MultiPeek<Lexer<'a, Token<'a>>>,
...
impl<'a> Parser<'a> {
    pub fn new() -> Self {
        Self {
            path: "".into(),
            lexer: itertools::multipeek(Token::lexer("")),
            expr_parser: ExprParser,
        }
    }
        ...
        while let Some(token) = self.lexer.peek() {
            let ast = match token {
                ...
                Token::Identifier(_) => {
                    if let Some(token) = self.lexer.peek() {
                        match token {
                            Token::LParan(_) => self.parse_fn_call(),
    ...
    pub fn set_source(&mut self, path: PathBuf, source: &'a str) {
        self.path = path;
        self.lexer = itertools::multipeek(Token::lexer(source));
    }
