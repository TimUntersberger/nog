...
unsafe extern "system" fn monitor_cb(hmonitor: HMONITOR, _: HDC, rect: LPRECT, data: LPARAM) -> BOOL {
...
pub fn fetch_displays() -> Vec<Display> {
    let mut temp: Vec<Display> = Vec::new();
    unsafe {
        EnumDisplayMonitors(
            std::ptr::null_mut(),
            std::ptr::null_mut(),
            Some(monitor_cb),
            &mut temp as *mut Vec<Display> as isize,
        );
    }
    temp
}
