...
impl Window {
    ...
    pub fn reset_style(&self) -> Result<(), util::WinApiResultError> {
        unsafe {
            util::winapi_nullable_to_result(SetWindowLongA(self.id as HWND, GWL_STYLE, self.original_style))?;
        }
    ...
    pub fn reset_pos(&self) -> Result<(), util::WinApiResultError> {
        unsafe {
            util::winapi_err_to_result(SetWindowPos(
                self.id as HWND, 
                std::ptr::null_mut(), 
    ...
    pub fn get_foreground_window() -> Result<HWND, util::WinApiResultError> {
        unsafe { 
            util::winapi_ptr_to_result(GetForegroundWindow())
        }
    }
    pub fn get_parent_window(&self) -> Result<HWND, util::WinApiResultError> {
        unsafe { 
            util::winapi_ptr_to_result(GetParent(self.id as HWND))
        }
    }
    pub fn get_style(&self) -> Result<i32, util::WinApiResultError> {
        unsafe { 
            util::winapi_nullable_to_result(GetWindowLongA(self.id as HWND, GWL_STYLE))
        }
    }
    ...
    pub fn to_foreground(&self) -> Result<(), util::WinApiResultError> {
        unsafe {
            util::winapi_err_to_result(SetForegroundWindow(self.id as HWND))?;
        }
    ...
    pub fn remove_title_bar(&self) -> Result<i32, util::WinApiResultError> {
        unsafe {
            util::winapi_nullable_to_result(SetWindowLongA(self.id as HWND, GWL_STYLE, 
                self.original_style & 
                !WS_CAPTION as i32 & 
