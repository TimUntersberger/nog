...
impl Default for TaskBarPosition {
    fn default() -> Self { TaskBarPosition::Bottom }
}
...
unsafe extern "system" fn enum_windows_cb(hwnd: HWND, _: LPARAM) -> BOOL {
    ...
    if is_task_bar {
        ...
        let is_display_primary = match DISPLAYS.lock().unwrap().iter_mut().find(|d| d.hmonitor == hmonitor) {
            Some(d) => {
...
fn get_taskbar_position(rect: RECT, hwnd: HWND, hmonitor: i32) -> TaskBarPosition {
    let mut monitor_info = MONITORINFO { cbSize: size_of::<MONITORINFO>() as u32, ..MONITORINFO::default() };
    unsafe {
        GetMonitorInfoA(hmonitor as HMONITOR, &mut monitor_info);
    }
    let RECT { left, right, top, bottom } = monitor_info.rcMonitor;
    let is_window_visible = unsafe { IsWindowVisible(hwnd) == 1 };

    if !is_window_visible { TaskBarPosition::Hidden }
    else if rect.left == left && rect.top == top && rect.bottom == bottom { TaskBarPosition::Left } 
    else if rect.right == right && rect.top == top && rect.bottom == bottom { TaskBarPosition::Right } 
    else if rect.left == left && rect.top == top && rect.right == right { TaskBarPosition::Top } 
    else { TaskBarPosition::Bottom }
}
